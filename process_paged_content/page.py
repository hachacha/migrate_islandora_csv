...
class Page(Const):
	...
	def write_page_headers(self):
		self.output_page_text_csv.write(self.page_text_header)
	...
	def write_page_line(self,title,page_num,parent,issued):
		...
		node_line = ("%s page %s|||%s|%s|%s|%s/%s/%s/img/page_%s.jpg\n") % (title, page_num,parent,issued,page_num,self.file_staging_url,self.collection,parent,page_num)
		self.output_page_nodes_csv.write(node_line)

		file_line = ("%s page %s|||%s|%s/img/page_%s.jpg\n") % (title, page_num,issued,parent,page_num)
		self.output_page_files_csv.write(file_line)
		# title|parent|weight|file
		text_line = ("%s page %s|%s|%s|%s/txt/page_%s.txt\n") % (title,page_num,parent,page_num,parent,page_num )
		self.output_page_text_csv.write(text_line)
