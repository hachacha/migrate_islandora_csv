...
class Book(Const):
	...
	def write_book_node(self,parent):
		...
		book_line = ("%s|%s||%s|%s pages; book|%s|%s|%s|%s|%s\n") % (self.title,self.subtitle,year,self.collection,self.num_of_pages,self.issued_date,self.subject,self.publisher,self.geo_location,parent)
